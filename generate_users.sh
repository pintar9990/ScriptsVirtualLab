...
echo -n "Introduce la contraseña que tendrá la cuenta de examen: "
read -s PASSWD
...
# ===========================================
# FUNCIÓN PARA CREAR/VERIFICAR USUARIO
# ===========================================
process_host() {
    local HOST=$1
    local ID=$2
    ...
    # Comando SSH para crear usuario si no existe
    ssh -T "root@$HOST" << EOF
        # Verificar si usuario existe
        if id '$EXAM_USER' &>/dev/null; then
            echo 'Usuario $EXAM_USER ya existe'
            
            # Cambiar contraseña por seguridad
            echo '$EXAM_USER:$PASSWD' | chpasswd
            ...
            # Quitar sudo si lo tiene
            if groups '$EXAM_USER' | grep -q -E '(sudo|wheel)'; then
                echo 'Quitando privilegios de sudo'
                usermod -G users '$EXAM_USER'
            fi
        else
            echo 'Creando usuario $EXAM_USER...'
            useradd -m -s /bin/bash -G users '$EXAM_USER'
            echo '$EXAM_USER:$PASSWD' | chpasswd
            passwd -l '$EXAM_USER'
            echo 'Usuario creado y bloqueado'
        fi
EOF
...
# EJECUCIÓN PRINCIPAL
# ===========================================
log_info "INICIANDO CREACIÓN DE USUARIO '$EXAM_USER'"
...
echo ""
log_info "Usuario '$EXAM_USER' creado con contraseña: $PASSWD"
log_info "Estado inicial: BLOQUEADO"
